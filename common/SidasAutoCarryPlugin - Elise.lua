...
function PluginOnLoad()
        ...
        ProdictE = Prodict:AddProdictionObject(_E, eRange, 1450, 0.250, 80, myHero, CastE)
        ProdictECol = Collision(eRange, 1450, 0.250, 80)
        for I = 1, heroManager.iCount do
                local hero = heroManager:GetHero(I)
                if hero.team ~= myHero.team then
                        --ProdictE:CanNotMissMode(true, hero)
        ProdictW = Prodict:AddProdictionObject(_W, wRange, 800, 0.250, 90, myHero, CastW)
...
function SpiderlingS()
      if WAble and AutoCarry.PluginMenu.useW and AutoCarry.PluginMenu.stunC and Stunned then
			ProdictW:EnableTarget(Target, true) end 
end  
